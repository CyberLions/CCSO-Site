<template>
  <div class="sponsors-page min-h-screen">
    ...
    <section class="sponsors-cont">
      ...
      <!-- Sponsors by Tier (built from configuration) -->
      <div v-else>
        <div v-for="section in tierSections" :key="section.key" class="resource">
          <h3>
            Thank you to our <span :style="{ color: section.color }">{{ section.label }}</span> Sponsors!
          </h3>
          <div class="tier-sponsors">
            <a
        ...
        <!-- Empty State -->
        <div v-if="!tierSections.length" class="resource">
...
// Tier configuration and builder
const tierConfig = [
  { key: 'gold', label: 'Gold', color: '#F4C267', order: 1 },
  { key: 'silver', label: 'Silver', color: '#C0C0C0', order: 2 },
  { key: 'bronze', label: 'Bronze', color: '#CD7F32', order: 3 },
  { key: 'resource', label: 'Resource', color: '#007BFF', order: 4 }
]

const { sponsors, loading, error, loadSponsors } = useSponsors()

const tierSections = computed(() => {
  const sections = tierConfig
    .map(cfg => ({
      ...cfg,
      sponsors: (sponsors?.value || []).filter(s => s.tier === cfg.key)
    }))
