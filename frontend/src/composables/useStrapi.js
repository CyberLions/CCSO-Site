...
export function useSponsors() {
  const { fetchSponsors, fetchSponsor, loading, error } = useStrapi()
  const sponsors = ref([])
  ...
  const loadSponsor = async (id, params = {}) => {
    const data = await fetchSponsor(id, params)
    currentSponsor.value = data?.data
    return currentSponsor.value
  }

  const getSponsorsByTier = (tier) => {
    return sponsors.value.filter(sponsor => sponsor.tier === tier)
  }

  const getGoldSponsors = () => getSponsorsByTier('gold')
  const getSilverSponsors = () => getSponsorsByTier('silver')
  const getBronzeSponsors = () => getSponsorsByTier('bronze')

  return {
    sponsors: computed(() => sponsors.value),
    currentSponsor: computed(() => currentSponsor.value),
    loading,
    error,
    loadSponsors,
    loadSponsor,
